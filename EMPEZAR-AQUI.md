# 🚀 EMPEZAR AQUÍ — SpecFounder v2

**¿Qué es esto?** Un asistente que te entrevista paso a paso y construye la **especificación** (el "spec") de lo que quieras crear: un sistema/app/API/web **o** una obra creativa (novela, serie de imágenes, guion de video, etc.). Va guardando el avance, así que si se cierra puedes continuar donde quedaste.

**No necesitas instalar nada ni saber programar.** Sigue 3 pasos.

---

## ✅ Los 3 pasos

1. **Copia** todo el bloque de prompt que está más abajo (entre las líneas `=== INICIO ===` y `=== FIN ===`).
2. **Pégalo** como primer mensaje en tu herramienta de IA (Claude, ChatGPT, Cursor, Codex, etc.).
3. **Responde** las preguntas. El asistente te guía: una pregunta a la vez, siempre con una recomendación. Cuando termine, te entrega tu spec listo.

> 💡 Si tu IA puede crear archivos en tu computadora (Claude Code, Cursor en modo agente, Codex…), creará una carpeta `.specfounder/` con tu progreso. Si usas un chat normal, te pedirá guardar un bloque de texto para poder retomar.

---

## ▶️ ¿Qué te va a preguntar al inicio?

1. **¿Para qué es el spec?** Código o Creativo (y el subtipo).
2. **Si es código:** qué metodología quieres de destino (OpenSpec, GitHub Spec-Kit o SDD genérico). *Si no sabes, elige "SDD genérico".*
3. **¿Es nuevo o ya existe?** Si ya existe material, lo revisa antes de preguntarte.
4. **Si es nuevo:** te ayuda a definir la **Visión** (te propone 3 alternativas y eliges), o pegas la tuya.

Luego te entrevista por las 6 secciones y al final genera tu spec (o tu "Biblia" si es una obra creativa).

---

## 📋 El prompt para copiar

Copia desde `=== INICIO ===` hasta `=== FIN ===`:
...
session.md (frontmatter mínimo): domain, methodology, project_mode, phase, current_section, current_question_id, sections.s1..s6 (pendiente|en_curso|completa), glossary_terms, adr_count, updated_at. Cuerpo: "Siguiente acción", "Ramas abiertas", "Log de decisiones", "Contradicciones resueltas".
...
1. Muestra SPEC.draft.md, CONTEXT.draft.md y las decisiones irreversibles para revisión final.
...
3. Genera el bloque de handoff: "Usa estos documentos como fuente de verdad absoluta. Los términos del glosario/canon son los únicos válidos. Antes de generar código/capítulos/imágenes/partes, verifica que no contradigan el spec. Si algo lo contradice, señálalo antes de proceder."
...
## 🔁 ¿Se cerró la sesión? Así continúas

Vuelve a abrir tu herramienta **en la misma carpeta** y di: *"Retoma la sesión de SpecFounder"*. Leerá `.specfounder/session.md`, te mostrará un resumen de lo avanzado y seguirá justo donde quedaste, sin repetir preguntas. (Si usabas un chat sin archivos, pega el bloque de progreso que guardaste.)
...
- **Instalación por herramienta** (Claude Code, Codex, OpenCode, Cursor, Antigravity): [specfounder-v2/integrations/](specfounder-v2/integrations/)
- **Versión multi-agente avanzada y perfiles de dominio:** [specfounder-v2/](specfounder-v2/)

> Este archivo es el atajo para principiantes. El prompt de arriba es una copia del prompt portable canónico ([specfounder-v2/monolith/specfounder-v2.monolith.md](specfounder-v2/monolith/specfounder-v2.monolith.md)); si editas las reglas, actualiza ambos.
